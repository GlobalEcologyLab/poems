...
- Fork the package and clone onto your computer. If you haven’t done
  ...
  `usethis::create_from_github("GlobalEcologyLab/poems", fork = TRUE)`.
...
- Make your changes, commit to git, and then create a PR by running
  ...
  PR should contain `Fixes #issue-number`. If your PR addresses an issue
  no one has reported yet, please create an issue to reference.
...
Please note that the poems project is released with a [Contributor Code
of Conduct](https://ropensci.org/code-of-conduct/). By contributing to
this project you agree to abide by its terms.

## Roadmap

`poems` development at this time is mostly geared toward support and
compatibility with the extensions of `poems`, which currently include
`paleopop` and `epizootic`, and may include more in the future.

We very much wish to avoid changes that break existing code, as five
published scientific articles are based on `poems` at time of writing,
and we would like the code that supports these papers to remain
reproducible.
