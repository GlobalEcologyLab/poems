...
Generator <- R6Class(
  ...
  public = list(
    ...
    initialize = function(
      ...
      if (is.null(generative_template)) {
        # Set any user-named parameters to attach to the template (via aliases)
        param_names <- names(c(list(...), list(...)$params))
        self$attribute_aliases <- c(self$attribute_aliases, list())
        for (param in param_names[
          !param_names %in%
            c("attribute_aliases", "params", private$.active_attributes)
        ]) {
    ...
    get_attributes = function(params = NULL) {
      ...
      if (
        any(c("error_messages", "warning_messages") %in% names(attribute_list))
      ) {
      ...
      if (!is.null(params)) {
        for (param in params[!params %in% names(attribute_list)]) {
          ...
          if (
            param %in%
              names(self$attribute_aliases) &&
              is.null(required_type) &&
              !(param %in%
                c(
                  names(self$file_templates),
                  names(self$function_templates),
                  names(self$distribution_templates)
                ))
          ) {
            attribute <- self$attribute_aliases[[param]]
            attribute_root <- unlist(strsplit(attribute, "$", fixed = TRUE))[1]
            attribute_root <- unlist(strsplit(
              attribute_root,
              "[",
              fixed = TRUE
            ))[1]
          ...
          if (
            (is.null(required_type) || required_type == "file") &&
              attribute_root %in% names(self$file_templates)
          ) {
            eval(parse(
              text = sprintf(
                "%s <- self$read_file(attribute_root)",
                attribute_root
              )
            ))
            attribute_list[[param]] <- eval(parse(text = attribute))
          } else if (
            (is.null(required_type) || required_type == "function") &&
              attribute_root %in% names(self$function_templates)
          ) {
            eval(parse(
              text = sprintf(
                "%s <- self$run_function(attribute_root)",
                attribute_root
              )
            ))
            attribute_list[[param]] <- eval(parse(text = attribute))
          } else if (
            (is.null(required_type) || required_type == "distribution") &&
              attribute_root %in% names(self$distribution_templates)
          ) {
            eval(parse(
              text = sprintf(
                "%s <- self$sample_distribution(attribute_root)",
                attribute_root
              )
            ))
            attribute_list[[param]] <- eval(parse(text = attribute))
      ...
      for (param in names(attribute_list)) {
        ...
        if (
          !is.null(self$region) &&
            any(
              class(attribute_list[[param]])[1] %in%
                c("RasterLayer", "RasterStack", "RasterBrick")
            ) &&
            !self$region$raster_is_consistent(attribute_list[[param]])
        ) {
          self$error_messages <- paste(
            "Region is inconsistent with template generated raster for attribute: ",
            param
          )
        }
        if (
          !is.null(attribute_list[[param]]) &&
            any(c(param, self$attribute_aliases[[param]]) %in% self$outputs) &&
            (is.numeric(attribute_list[[param]]) ||
              class(attribute_list[[param]])[1] %in%
                c("data.frame", "RasterLayer", "RasterStack", "RasterBrick"))
        ) {
          # Apply decimal rounding
          if (!is.null(self$decimals)) {
            attribute_list[[param]][] <- round(
              ...
              if (
                self$generate_rasters &&
                  nrow(as.matrix(attribute_list[[param]])) ==
                    self$region$region_cells
              ) {
                # array to raster
                attribute_list[[
                  param
                ]] <- self$region$raster_from_values(attribute_list[[param]])
              }
            }
          }
          ...
          if (!is.null(self$occupancy_mask)) {
            if (
              length(as.matrix(attribute_list[[param]][])) <
                length(as.matrix(self$occupancy_mask[]))
            ) {
              if (
                nrow(as.matrix(attribute_list[[param]][])) ==
                  nrow(as.matrix(self$occupancy_mask[])) &&
                  ncol(as.matrix(attribute_list[[param]][])) == 1
              ) {
                # use first column of mask
                attribute_list[[param]][] <- attribute_list[[param]][] *
                  self$occupancy_mask[][, 1]
      ...
      if (any(c("error_messages", "warning_messages") %in% params)) {
        attribute_list <- c(
          attribute_list,
          super$get_attributes(c("error_messages", "warning_messages"))
        )
      }
    ...
    generate = function(input_values = list()) {
      if (
        (is.null(self$inputs) && length(input_values)) || is.null(self$outputs)
      ) {
        missing <- c("inputs", "outputs")[which(c(
          is.null(self$inputs) && length(input_values),
          is.null(self$outputs)
        ))]
        return(list(
          error_messages = sprintf(
            "%s generation requires settings for: %s",
            self$description,
      ...
      for (param in self$inputs) {
        if (
          any(
            c(param, self$get_attribute_aliases(param)) %in% names(input_values)
          )
        ) {
      ...
      if (inputs_present == length(self$inputs)) {
        ...
        if (all(unlist(generative_requirements_satisfied))) {
          # ensure the required file, function, and/or distribution templates are set for specified outputs
          return(self$new_clone(params = input_values)$get_attributes(c(
            self$outputs,
            "error_messages",
            "warning_messages"
          )))
          ...
          return(list(
            error_messages = sprintf(
              "%s generation requires further settings for output(s): %s",
              self$description,
              paste(
                names(which(!unlist(generative_requirements_satisfied))),
                collapse = ", "
              )
            )
          ))
        ...
        return(list(
          error_messages = sprintf(
            "%s generation requires inputs: %s",
            self$description,
            paste(self$inputs, collapse = ", ")
          )
        ))
      }
    },
    ...
    add_file_template = function(
      ...
      if (is.character(param) && is.character(path_template)) {
        if (
          length(path_params) ==
            (length(strsplit(paste0("#", path_template, "#"), "%s")[[1]]) - 1)
        ) {
          ...
          stop(
            "Ensure the path template contains a corresponding %s for each path parameter",
            call. = FALSE
          )
        }
      } else {
        stop(
          "The parameter name and path template should be strings",
          call. = FALSE
        )
      }
    },
    ...
    add_function_template = function(param, function_def, call_params = c()) {
      ...
      if (is.character(function_def)) {
        if (
          file.exists(function_def) &&
            length(grep(".R", toupper(function_def), fixed = TRUE))
        ) {
          # from file
          tryCatch(
            {
      ...
      if (!is.function(function_def)) {
        ...
        stop(
          "The function definition should have one argument, e.g. function(params)",
          call. = FALSE
        )
      }
    ...
    add_distribution_template = function(
      param,
      ...
      if (
        !is.null(distr_type) &&
          (!is.character(distr_type) ||
            !(distr_type %in%
              c("uniform", "normal", "lognormal", "beta", "triangular")))
      ) {
        stop(
      ...
      if (is.list(expected_distr_params)) {
        if (
          !is.list(distr_params) ||
            !(all(expected_distr_params[[1]] %in% names(distr_params)) ||
              all(expected_distr_params[[2]] %in% names(distr_params)))
        ) {
          stop(
            sprintf(
              "The distribution parameters should be a list containing %s distribution parameters: %s (or %s)",
              distr_type,
              paste(expected_distr_params[[1]], collapse = ", "),
              paste(expected_distr_params[[2]], collapse = ", ")
            ),
            call. = FALSE
          )
        }
        ...
        !is.list(distr_params) ||
          !all(expected_distr_params %in% names(distr_params))
      ) {
        stop(
          sprintf(
            "The distribution parameters should be a list containing %s distribution parameters: %s",
            distr_type,
            paste(expected_distr_params, collapse = ", ")
          ),
          call. = FALSE
        )
      }
      if (
        !is.null(sample) &&
          !(all(
            unlist(lapply(sample, is.numeric)) |
              unlist(lapply(sample, is.character))
          ))
      ) {
        #
        stop(
          "The sample attribute(s) should be parameter name string(s) or numeric",
          call. = FALSE
        )
      }
    ...
    read_file = function(param) {
      if (param %in% names(self$file_templates)) {
        ...
        if (all(path_params %in% names(path_param_list))) {
          file_path <- eval(parse(
            text = paste0(
              "sprintf(path_template, ",
              paste0(
                sprintf("path_param_list[[%d]]", 1:length(path_param_list)),
                collapse = ", "
              ),
              ")"
            )
          ...
          missing_params <- path_params[which(
            !path_params %in% names(path_param_list)
          )]
          self$error_messages <- paste(
            "Missing parameter(s) for file template: ",
    ...
    run_function = function(param) {
      if (param %in% names(self$function_templates)) {
        ...
        if (all(call_params %in% names(call_param_list))) {
          value_list <- list()
          value_list[[param]] <- self$function_templates[[param]]$function_def(
            call_param_list
          )
          if (!param %in% self$model_attributes) {
            self$set_attributes(value_list)
          ...
          missing_params <- call_params[which(
            !call_params %in% names(call_param_list)
          )]
          self$error_messages <- paste(
            "Missing parameter(s) for function template: ",
            paste(missing_params, collapse = ", ")
          )
          return(NULL)
        }
    ...
    sample_distribution = function(param) {
      if (param %in% names(self$distribution_templates)) {
        ...
        if (is.list(sample) && all(sample_names %in% c("mid", "window"))) {
          ...
          if (is.numeric(sample$mid) && is.numeric(sample$window)) {
            sample <- c(
              ...
              min(sample$mid + sample$window / 2, 1)
            )
          } else {
            self$error_messages <- sprintf(
              "The distribution sample for %s utilizes missing parameter(s): %s",
              param,
              paste(
                unlist(self$distribution_templates[[param]][["sample"]][which(
                  !sample_names %in% names(sample)
                )]),
                collapse = ", "
              )
            )
          ...
          # single or c(lower, upper) bounds
          ...
          if (any(is.na(sample))) {
            self$error_messages <- sprintf(
              "The distribution sample for %s utilizes missing parameter(s): %s",
              param,
              paste(self$distribution_templates[[param]][[
                "sample"
              ]][which(is.na(sample))])
            )
            return(NULL)
        ...
        normalize_threshold <- self$distribution_templates[[param]][[
          "normalize_threshold"
        ]]
        ...
        if (!is.null(sample) && !all(sample >= 0 & sample <= 1)) {
          self$error_messages <- sprintf(
            "The distribution sample for %s should utilize existing parameters with values between 0 and 1 (inclusive)",
            param
          )
          return(NULL)
        }
        ...
        if (self$uses_correlations) {
          ...
          if (is.character(correlated_deviates)) {
          ...
          if (length(sample) > 1) {
            generated_samples <- sample[1] +
              pnorm(correlated_deviates) * (sample[2] - sample[1])
          } else {
            # use 0-1
        ...
        for (distr_param in names(distr_params)) {
          if (is.character(distr_params[[distr_param]])) {
            distr_param_values[[
              distr_param
            ]] <- self$get_attributes(distr_params[[
              distr_param
            ]])[[distr_params[[distr_param]]]]
          } else {
          ...
          if (!is.null(distr_param_values[[distr_param]])) {
            if (
              !is.null(self$region) &&
                ...
                any(
                  class(distr_param_values[[distr_param]]) %in%
                    c("RasterLayer", "RasterStack", "RasterBrick")
                )
            ) {
              distr_param_values[[distr_param]] <- distr_param_values[[
                distr_param
              ]][self$region$region_indices]
            }
            if (distr_type %in% c("uniform", "triangular")) {
              distr_param_values[[distr_param]][which(
                !is.finite(distr_param_values[[distr_param]])
              )] <- NA
            } else {
              distr_param_values[[
                distr_param
              ]][which(is.nan(distr_param_values[[distr_param]]))] <- NA
            }
          }
        }
        ...
        if (!all(names(distr_params) %in% names(distr_param_values))) {
          missing_values <- names(distr_params)[which(
            !names(distr_params) %in% names(distr_param_values)
          )]
          self$error_messages <- sprintf(
        ...
        switch(
          distr_type,
          uniform = generated_values <- stats::qunif(
            generated_samples,
            min = distr_param_values$lower[],
            max = distr_param_values$upper[]
          ),
          normal = generated_values <- stats::qnorm(
            generated_samples,
          ...
          lognormal = {
            if (all(c("mean", "sd") %in% names(distr_param_values))) {
              # transform
              distr_param_values$meanlog <- log(
                distr_param_values$mean^2 /
                  sqrt(distr_param_values$mean^2 + distr_param_values$sd^2)
              )
              distr_param_values$sdlog <- sqrt(log(
                1 + distr_param_values$sd^2 / distr_param_values$mean^2
              ))
            }
            # Get shape of generated values and indices of available values
            generated_values <- (generated_samples +
              distr_param_values$meanlog[] +
              distr_param_values$sdlog[])
            ...
            distr_param_values <- lapply(
              distr_param_values,
              function(x) x * generated_values
            )
            if (all(c("mean", "sd") %in% names(distr_param_values))) {
            ...
            generated_values[available_indices] <- stats::qlnorm(
              generated_samples[available_indices],
          ...
          beta = {
            if (all(c("mean", "sd") %in% names(distr_param_values))) {
              # transform
              distr_param_values$sd <- array(
                distr_param_values$sd,
                dim(as.matrix(distr_param_values$mean * distr_param_values$sd))
              )
              finite_indices <- which(is.finite(
                distr_param_values$mean * distr_param_values$sd
              ))
              # Limit sd to < sqrt(mean*(1 - mean))
              distr_param_values$sd[finite_indices] <- pmin(
                distr_param_values$sd[finite_indices],
                sqrt(
                  distr_param_values$mean[finite_indices] *
                    (1 - distr_param_values$mean[finite_indices])
                ) *
                  0.999
              )
              distr_param_values$alpha <- distr_param_values$mean *
                (distr_param_values$mean *
                  (1 - distr_param_values$mean) /
                  distr_param_values$sd^2 -
                  1)
              distr_param_values$beta <- (1 - distr_param_values$mean) *
                (distr_param_values$mean *
                  (1 - distr_param_values$mean) /
                  distr_param_values$sd^2 -
                  1)
            ...
            # Get shape of generated values and indices of available values
            generated_values <- (generated_samples +
              distr_param_values$alpha[] +
              distr_param_values$beta[])
            available_indices <- which(!is.na(generated_values))
            generated_values[] <- 1
            # Set the shape of the samples and distribution parameters consistently
            generated_samples <- generated_samples * generated_values
            ...
            generated_values[available_indices] <- stats::qbeta(
              generated_samples[available_indices],
          ...
          triangular = {
            ...
            generated_values <- (generated_samples +
              distr_param_values$lower[] +
              distr_param_values$upper[] +
              distr_param_values$mode[])
            ...
            finite_values <- qtri(
              generated_samples[finite_indices],
            ...
            flat_indices <- which(
              distr_param_values$lower[finite_indices] >=
                distr_param_values$upper[finite_indices]
            )
            finite_values[flat_indices] <- distr_param_values$upper[
              finite_indices
            ][flat_indices]
        ...
        if (!is.null(self$region) && self$generate_rasters) {
          value_list[[param]] <- self$region$raster_from_values(
            generated_values
          )
        } else {
    ...
    add_generative_requirements = function(params = list(), ...) {
      ...
      if (length(invalid_params)) {
        # return a message for invalid parameters
        ...
        for (param in names(invalid_params)) {
          return_message <- paste0(
            return_message,
            (sprintf(" %s = '%s' not added;", param, invalid_params[[param]]))
          )
        }
          ...
          if (!is.null(value)) {
            if (
              !is.null(value$region) &&
                !is.null(self$region) &&
                !identical(value$region, self$region)
            ) {
              stop(
      ...
      if (missing(value)) {
        ...
        if (!is.null(value)) {
          ...
          if (value && is.null(self$region)) {
            stop(
              "Rasters can only be generated when a region is defined that uses rasters",
              call. = FALSE
            )
          }
        }
