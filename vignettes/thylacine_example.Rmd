...
spatial differences in habitat suitability decline. Finally, in step 6, we will utilize
...
density dependence function will operate at a neighborhood level, where each 
...
```{r, message = FALSE}
# Define neighborhoods (of up to 9 cells) based on a 14 km range from each grid cell
...
effect to suppress sustained low, non-viable populations. Additional functionality is also 
...
We could perform more thorough statistical analyzes to verify our model ensemble. 
However, from our comparative plots of *thylacine* bounty submissions we have gained 
...
better model spatio-temporal responses to the bounty reward. With an improved model we 
could gain insight into spatio-temporal dynamics of the *thylacine* decline. 

##### Model ensemble usage
Although our model ensemble could be improved to better match spatio-temporal patterns, 
...
Generally, we found that the spatio-temporal patterns of the historic response to the 
...
the purpose of our, mostly qualitative visualization-based, analyzes of our *thylacine* 
...
for building your own customized models with *poems*, to explore the spatio-temporal 
