...
the bounty interval, derived from historic bounty, capture and sighting records.
...
Now that we have selected our model ensemble we can now examine:

* how well our summary metric targets were matched;
...
We can also verify our model via historic bounty record data not explicity utilized in
our validation and model selection. 

Lastly, we can use our model ensemble to examine the spatiotemporal extirpation pattern 
...
as a final demonstration let's examine how we can use our ensemble to examine our 
simulated spatiotemporal extirpation pattern. 
...
1. First we delved into constructing a customized model with user-defined density 
   dependence and harvest functions. 
1. We then built generators for dynamically generating stage matrices, initial 
   abundances, carrying capacities, and dispersal rates for our sampled models. 
...
We then examined how well our model ensemble matched our target metrics, and analysed 
...
that was not explicitly used in the model validation and ensemble selection.

Generally, we found that the spatiotemporal patterns of the historic response to the 
bounty reward were not well-matched by our model ensemble simulations, although the 
congruence with expected patterns at the Tasmania-wide level was reasonable. However,
the purpose of our, mostly qualitative visualization-based, analyses of our *thylacine* 
model was to demonstrate *poems* features, rather than to produce a definitive model 
simulating the decline of the *thylacine*.

In performing these steps we explored much of the functionality offered by *poems*. We 
hope that this vignette has provided you with a feature-rich example to use as a guide 
for building your own customized models with *poems*, to explore the spatiotemporal 
dynamics of populations of interest in your research and practice.
