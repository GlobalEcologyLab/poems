...
It was claimed that up to half of *thylacines* captured were not submitted for bounty 
...
flexibility in how we combine our individual metrics, or how we deal with *NA* values 
(e.g. no extirpation) in step 6, we can alternatively store our metrics for each slope 
interval or bioregion in generated  matrix rows and combine the values later. Here we 
will demonstrate both approaches, although the matrix approach requires more disk space.
...
We will also calculate the error from the confidence interval (CI) for our simulated 
total extinction dates for our full metrics set.
...
and IBRA extirpation) and error metric for total extinction (CI). Note that for models 
where our simulated *thylacine* persist until the end of the simulation (1967), the 
extinction (CI error) value will be *NA*. Since the *abc* function requires finite 
values only, we will use the *non_finite_replacements* attribute of the validator to 
replace these *NA* values with the CI error corresponding to 1968.
...
Let's now plot the distributions of our summary metrics of our selected model ensemble, 
as well as those of the full 20,000 model simulations, to examine how well our 
individual (uncombined) metric targets have been matched.
...
From this plot we can observe that, although the summary metrics of all 20,000 sample 
models generally do not align well with our bounty slope targets, our bounty slope 
metrics have been selected toward their corresponding targets. Generally, the 
distribution our full sample set does not facilitate model congruence with the targets,
suggesting that our harvest function may be inadequate for generating the expected 
temporal contour of the total *thylacine* bounty.

```{r, message = FALSE, fig.align = "center", fig.width = 6, fig.height = 8}
...
graphics::boxplot(cbind(extirpation[selected_indices, 10:1], NA),  horizontal = TRUE, 
                  ...
                  add = TRUE, col = NULL)
graphics::points(x = targets$ibra_extirpation, y = 10:2, col = "red", pch = 15)
...
Here we see that, although our total Tasmania-wide extirpation date matches well, the
simulated extirpation dates for individual IBRA bioregions vary in their congruence 
with the target values. We may wish assess the degree to which our sampled model 
harvest parameters can influence the spatial dynamics of the *thylacine* decline.

##### Model ensemble analysis
We can now examine the features of our selected model ensemble to analyse its properties
...
We could also explore relationships between selected model parameters, thus identifying
...
the posterior distributions of our 200 selected model parameters (in red) relative to 
those of all 20,000 simulations (in black), and their prior distributions (dashed 
line). This can give us further insight into the influence of each model parameter has 
had on our model selection. 

From these diagnostics and plots we can analyse and assess the adequacy of our model 
and/or summary metrics. We may choose to refine our model parameter ranges or our 
summary metrics. We may also choose to simulate a more extensive parameter space, 
...
hist(rep(ibra_extirpation_error_r, 2), breaks = 40, col = "gold3", add = TRUE)
...
an impact on our model validation and ensemble selection. Generally, the distributions
of the metrics have widened in our model ensemble replicate re-runs. We could further 
analyze the effects of stochasticity by exploring approaches for identifying which of 
our selected models are congruent with our target metrics most frequently.

##### Verifying and using the model ensemble
We can verify the adequacy of our model by examining how well it produces observed data 
patterns that were not (explicitly) utilized in the model validation and selection 
stages. While the slopes of the bounty were used for validating and selecting our model 
ensemble, the actual bounty magnitudes were not. Let's now use our model ensemble to 
compare our simulated bounty submitted, to the historical (time-series) records 
(provided with the *poems* package). Here we will use the selected model weights to 
calculate a weighted average of simulated bounty submitted for the selected ensemble 
...
```{r, message = FALSE, fig.align = "center", fig.width = 6.5, fig.height = 4}
...
We could perform more thorough statistical analyses to verify our model ensemble. 
However, from our comparitive plots of *thylacine* bounty submissions we have gained 
...
better model spatiotemporal responses to the bounty reward. With an improved model we 
could gain insight into spatiotemporal dynamics of the *thylacine* decline. 

Although our model ensemble could be improved to better match spatiotemporal patterns, 
as a final demonstration let's examine its spatiotemporal extirpation pattern. 
...
extirpation_raster <- region$region_raster
extirpation_raster[region$region_indices] <- weighted_extirpation
...
advanced demonstration of the *poems* workflow and its modules. We have delved into
constructing a customized model with user-defined density dependence and harvest 
functions. We built generators for dynamically generating stage matrices, initial 
abundances, carrying capacities, and dispersal rates for our sampled models. We ran
multiple Latin hypercube sampled models via a *poems* simulation manager. We then 
collated and calculated summary metrics, based on functions we defined within a results
manager. After which, we fed our sample model parameters, our summary metrics, and 
target values for these metrics (derived from historical bounty and sighting records),
into an ABC validator, and ran it to select our model ensemble. We then examined how
well our ensemble matched our target metrics, and analysed the influence that our model
parameters and stochasticity had on the ensemble selection. We also verified our model 
via the bounty records. Generally, we found that the spatiotemporal patterns of the 
historic response to the bounty reward were not well-matched by our model ensemble 
simulations, although the congruence with expected patterns was at the Tasmania-wide 
level was reasonable. 

Although our *thylacine* model is sufficient for demonstration purposes, we hope that 
this vignette has provided you with a feature-rich example to use as a guide for 
building your own customized models to explore the spatiotemporal dynamics of
populations of interest in your research and practice.
