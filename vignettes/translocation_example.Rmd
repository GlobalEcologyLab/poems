...
Create a model template using the *PopulationModel* class. This model is spatially explicit, 
so is defined using the *Region* class. In addition we make the model **temporally** explicit so we can introduce populations at defined time steps.

#### Study region
First, we'll define our study region. For this example, we utilise a `raster::Raster` of Thylacine habitat suitability.
...
# Project the region
tasmania_raster <- projectRaster(tasmania_raster, tempExt, method = "ngb")
plot(tasmania_raster, main = "Tasmania raster", legend = FALSE,
...
int_point <- region$region_indices[which(region$region_indices == cellFromXY(tasmania_raster, xy = int_ll))]
...
#### Land-use modifier
Here we read in a land-use modifier layer which we can use to make our region spatiotemporally explicit. 
This has the effect of altering the HS values through time which causes dynamic changes in population abundance.
...
ibra_polygons@data <- merge(ibra_polygons@data, ibra_data, by.x = "layer", by.y = "index")
ibra_polygons
...
# seed is set to keep example results constanst
...
plot(rmax_regional, colNA = "grey75",
     ...
     zlim = range(rmax),
     addfun = function() {
       plot(ibra_polygons, border = hcl.colors(9, "Lajolla"), col = NA, add = TRUE)
       text(ibra_polygons, labels = rmax, halo = TRUE)
     },
     col = hcl.colors(100, "Zissou"))

# set upper and lower growth rates per region
ibra_rmax <- cbind(ibra_data, rmax_lower = round(rmax*0.5,2),
...
Here we define the custom `translocation` function
...
This genertor controls dispersal across our region based on mean dispersal distance and proportion of dispersers. 
...
# looks very similar to the dispersal curve in Storm et al. 1976
...
The capacity generator is setup in such a way that it requires multiple parameters to work.

1.`max_dens`: the maximum theoretical density of populations
1.`q_thresh`: the quantile threshold used to rescale the land-use modifier
1.`trans_n`: the number of animals that are introduced. This value is consistent with the `translocation` function.
...
generated_k <- capacity_gen$generate(input_values = list(max_dens = 100, q_thresh = 0.90, trans_n = 60))
...
model_template <- PopulationModel$new(
  ...
  dispersal_source_n_k = list(threshold = 0.92, cutoff = 0), ## dispersal_n_k_threshold; sampled.
...
We can see from the `diff_ras` above that the difference between timesteps 2 and 3 of the two model runs is 75 individuals, which was the value passed through to the `translocation` function of the original model run.

And now we can plot the difference.
```{r}
plotmin <- round_any(min(values(diff_ras), na.rm = TRUE), 10, floor)
plotmax <- round_any(max(values(diff_ras), na.rm = TRUE), 10, ceiling)
plot(diff_ras,
     zlim = c(plotmin, plotmax),
     breaks = c(plotmin, -100, -50, -20, -10, 0, 10, 20, 50, 100, plotmax),
     col = hcl.colors(11, "PuOr"),
     colNA = "grey75",
     addfun = function() {
...
For our example we only generate 10 samples. Typically however, a user would need to generate thousands to tens of thousands, of samples.
...
# Habitat suitability
lhs_gen$set_uniform_parameter("q_thresh", lower = 0.90, upper = 0.99, decimals = 2) # HS threshold
...
lhs_gen$set_uniform_parameter("dispersal_b", lower = 5, upper = 40, decimals = 0) # mean dispersal between 5 and 40 km
...
## 1 km2 = 50 per grid cell = (1*(10*10))*0.8 # assuming 80% grid cell used
...
{set.seed(54612)
  sample_data$UniqueID <- paste0(stri_rand_strings(nrow(sample_data), 4, "[A-Z]"),
...
We we can run a simulation for each set (or row) of sampled parameters. The
...
# Takes 15 seconds to run 10 example sims on a single core.
system.time({
...
Now that the simulations have run, we can extract the modelled abundances.
...
matrices (one row of values per simulation) that we may be interested in examining. 

We can see from above that the `run_output$summary` and `run_output$full_log` shows that 
there were a number of simulations that didn't complete successfully:
...
These simulations had a range of parameter values that caused the simulation to "fall-over".
These errors could be fixed by improving the `translocation` function, or alternatively the user could discard the simulations as being structurally wrong - i.e. that specific combination of parameters is simply unsuitable. We're going to treat them as the latter.
...
From the results we can see that our `translocation` function was working correctly, and that our `capacity_gen` was defined correctly, as the abundances in the first time step are the same as the 
`trans_n` values from our latin-hypercube samples.
...
Let's assume that samples 2, 3, 4, 8, and 9 are our "best" simulations after some sort of validation.
We have already extracted the abundances from these simulations, so now with a few more lines of code, we
can generate averages of the simulations as rasters
