...
`raster::Raster` of Thylacine habitat suitability.
...
values through time which causes dynamic changes in population
abundance.
...
This genertor controls dispersal across our region based on mean
...
The capacity generator is setup in such a way that it requires multiple
parameters to work.

1.`max_dens`: the maximum theoretical density of populations
1.`q_thresh`: the quantile threshold used to rescale the land-use
modifier 1.`trans_n`: the number of animals that are introduced. This
value is consistent with the `translocation` function.
...
timesteps 2 and 3 of the two model runs is 75 individuals, which was the
...
We can see from above that the `run_output$summary` and
`run_output$full_log` shows that there were a number of simulations that
...
Let's assume that samples 2, 3, 4, 8, and 9 are our "best" simulations
