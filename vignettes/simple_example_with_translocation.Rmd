...
# expand U Island to have an outer ring
template_raster <- extend(template_raster, y = c(1, 1), value = 1)
template_raster[!is.na(template_raster)] <- seq_len(sum(!is.na(template_raster[])))
...
raster::plot(region$region_raster, main = "Example region (cell indices)",
             ...
             colNA = "blue", 
             addfun = function() {
               text(region$region_raster, labels = region$region_indices)
             })
```
...
Let's define a simple translocation function to introduce new populations of individuals at specified locations and times.

```{r message=FALSE}
# which region cells correspond to pops 5, 29, 42
idx <- which(region$region_indices %in% c(5, 29, 42))
intro_cells <- c(idx)
intro_times <- c(2, 4, 7)

cbind(intro_cells, intro_times)

# function for introducing new pops
translocation <- list(
  ...
  trans_n = 50, # introduction abundances. If not provided to model == 50
...
model_template <- PopulationModel$new(region = region,
                                      time_steps = 10, # years
                                      populations = region$region_cells, # 31. 7 on U Island
...
example_hs_raster[region$region_indices] <- {set.seed(54);rbeta(length(region$region_indices),
                                                  shape1 = 3, shape2 = 2)}
example_hs_raster <- raster::stack(replicate(model_template$time_steps, example_hs_raster),
                                   quick = TRUE)
raster::plot(example_hs_raster[[1:3]],
             xlab = "Longitude (degrees)", ylab = "Latitude (degrees)",
             zlim = c(0, 1),
             colNA = "blue",
             addfun = function() {
...
So we don't have populations on the ring around U Island, we explicitly force these cells to have NA values at the first time step. After the first time step, the values are set back to their original values. As we have no suitable values on the ring in the first time step, and there is no way for populations to disperse to the ring, the only way for populations to get there is to be "translocated" there by our `translocation` function. We could also set these to `NA` inside the carrying capacity generator below if we wanted.

```{r message=FALSE}
# set cells on the ring around U Island to NA for the first time step
vals <- example_hs_raster[[1]][c(17, 19, 24, 26, 31:33)]
example_hs_raster[[1]] <- NA
example_hs_raster[[1]][c(17, 19, 24, 26, 31:33)] <- vals
raster::plot(example_hs_raster[[1:3]],
             xlab = "Longitude (degrees)", ylab = "Latitude (degrees)",
             zlim = c(0, 1),
             colNA = "blue",
...
capacity_gen <- Generator$new(description = "Capacity generator",
                              example_hs = example_hs_raster[region$region_indices], # template attached as matrix
...
capacity_gen$add_function_template("initial_abundance",
                                   function_def = function(params) {
                                     # generate initial abundances using
                                     # only cells on U Island
                                     init_prob <- params$example_hs[, 1]
                                     idx <- which(is.na(init_prob))
                                     init_prob[is.na(init_prob)] <- 0
                                     distr_a <- stats::rmultinom(1, size = params$initial_n,
                                                      prob = init_prob)[, 1]
                                     distr_a[idx] <- 0
                                     return(distr_a)
                                     },
...
capacity_gen$add_function_template("carrying_capacity",
                                   function_def = function(params) {
                                     distr_k <- params$density_max*params$example_hs
                                     distr_k[is.na(distr_k[, 1]), 1] <- 0
                                     return(round(distr_k))
                                   },
                                   call_params = c("density_max", "example_hs"))
test_capacity <- capacity_gen$generate(input_values = list(initial_n = 500, density_max = 100)) # test

# plot initial abundance
plot(region$raster_from_values(test_capacity$initial_abundance),
     main = "Initial Abundances")

# plot first 3 carrying capacity
plot(region$raster_from_values(test_capacity$carrying_capacity[, 1:3]),
     addfun = function() {
       text(region$region_raster, labels = region$region_indices)
       })
...
test_dispersal <- dispersal_gen$generate(input_values = list(dispersal_p = 0.5,
                                                             dispersal_b = 700))
...
distance between those cells is greater than 3000 m. This is the same process that stops
dispersal to the ring we have created around U Island.

### Step 3: Run a single replicate of the template model
We can now run a single simulation to check that our model works correctly.


First we will run the template model for a single replicate without the `translocation` function
```{r, message = FALSE}
model_no_trans <- model_template$clone()

# for the single model we need to provide intitial values for 
# initial abundance, carrying capacity, and dispersal
## set.seed for reproducibility
{set.seed(343)
  test_capacity_gen <- capacity_gen$generate(input_values = list(initial_n = 350, 
                                                                 density_max = 100))
  test_dispersal <- dispersal_gen$generate(input_values = list(dispersal_p = 0.67,
                                                               dispersal_b = 760))
}

plot(region$raster_from_values(test_capacity_gen$initial_abundance))
plot(region$raster_from_values(test_capacity_gen$carrying_capacity[, 1:3]))

model_no_trans$set_sample_attributes(
  initial_abundance = test_capacity_gen$initial_abundance,
  carrying_capacity = test_capacity_gen$carrying_capacity,
  dispersal = test_dispersal,
  growth_rate_max = 0.4,
  harvest_rate = 0.3)

model_no_trans$is_complete()
results <- population_simulator(model_no_trans) # run poems simulator
```

Now that the simulation has run, we can plot some of the outputs to test that it's
working as expected.

```{r, message = FALSE}
# plot of total abundance and harvest
plot(x = 1:10, y = results$all$abundance,
     xaxt = "n", yaxt = "n",
     ylim = range(pretty(c(0, max(results$all$abundance)))),
     type = "l", xlab = "Time", ylab = "Total")
lines(x = 1:10, y = results$all$harvested,
      col = "firebrick")
axis(1, at = 1:10)
axis(2, at = pretty(c(0, max(results$all$abundance))))
legend("bottomleft", c("Total Abundance", "Total Harvest"),
       lty = 1, col = c("black", "firebrick"),
       bty = "n")

# Maps of total abundance
total_abund <- region$raster_from_values(results$abundance[, seq(1, 10, 2)])
plot_max <- max(values(total_abund), na.rm = TRUE)
raster::plot(total_abund,
             xlab = "Longitude (degrees)", ylab = "Latitude (degrees)",
             zlim = c(0, plot_max),
...
```
```{r, message = FALSE}
model_trans <- model_template$clone()

plot(region$raster_from_values(test_capacity_gen$initial_abundance))
plot(region$raster_from_values(test_capacity_gen$carrying_capacity[, 1:3]))

model_trans$set_sample_attributes(
  initial_abundance = test_capacity_gen$initial_abundance,
  carrying_capacity = test_capacity_gen$carrying_capacity,
  dispersal = test_dispersal,
...
model_trans$is_complete()
results_trans <- population_simulator(model_trans) # run poems simulator
```
...
```{r, message = FALSE}
# plot of total abundance and harvest
plot(x = 1:10, y = results_trans$all$abundance,
     xaxt = "n", yaxt = "n",
     ylim = range(pretty(c(0, max(results_trans$all$abundance)))),
     type = "l", xlab = "Time", ylab = "Total")
lines(x = 1:10, y = results_trans$all$harvested,
      col = "firebrick")
axis(1, at = 1:10)
axis(2, at = pretty(c(0, max(results_trans$all$abundance))))
abline(v = intro_times, lty = 2, col = "black")
legend("topright", c("Total Abundance", "Total Harvest"),
       lty = 1, col = c("black", "firebrick"),
       bty = "n")

# Maps of total abundance
total_abund_trans <- region$raster_from_values(results_trans$abundance)
plot_max <- max(values(total_abund_trans), na.rm = TRUE)
raster::plot(total_abund_trans,
             xlab = "Longitude (degrees)", ylab = "Latitude (degrees)",
             zlim = c(0, plot_max),
             col = hcl.colors(100, "Batlow"),
             colNA = "grey75")
...
### Step 4: Sample model and generator parameters for each simulation
...
We also need to include a parameter for the number of translocated individuals.

In our simple `translocation` function, all cells have the same number of individuals released at
their respective time steps. It would be a relatively simple matter to alter the function to read from
a matrix that contained `Time Step | Cell | Number Individuals`, to allow site specific 
abundances to be translocated/introduced.
...
### Step 5: Build a simulation manager to run each simulation
