test_that("initialization and parameter setting", {
  ...
  expect_equal(
    dispersal_gen$get_attributes(c("description", "inputs", "outputs")),
    list(
      description = "dispersal", inputs = c("dispersal_proportion", "dispersal_breadth", "dispersal_max_distance"),
      outputs = c("dispersal_data")
    )
  )
  dispersal_gen <- DispersalGenerator$new(coordinates = coordinates)
  expect_warning(
    dispersal_gen <- DispersalGenerator$new(region = Region$new()),
...
test_that("calculate distance data", {
  ...
  dispersal_gen$set_distance_classes(minimum = 100, maximum = 400, interval = 20)
  expect_error(
    dispersal_gen$calculate_distance_data(distance_matrix = distance_matrix[1:15, 1:15]),
    "Distance matrix dimensions must be consistent with region/coordinates"
  )
  expect_error(
    dispersal_gen$calculate_distance_data(distance_matrix = distance_matrix * 10),
  ...
  # Consistency
  expect_error(
    dispersal_gen$set_distance_classes(minimum = 100, maximum = 450, interval = 20),
...
test_that("calculate dispersals", {
  ...
  # Compact
  expect_equal(
    max(dispersal_gen$dispersal_data[[1]][c("emigrant_row", "immigrant_row")]),
...
test_that("connect dispersal friction object", {
  ...
  expect_error(
    dispersal_gen$dispersal_friction <- "dummy",
  ...
  expect_error(
    dispersal_gen$dispersal_friction <- DispersalFriction$new(coordinates = coordinates[1:15, ]),
    "Dispersal friction object is inconsistent with the dispersal generator region/coordinates"
  )
  expect_error(
    dispersal_gen$dispersal_friction <- DispersalFriction$new(conductance = array(1, c(15, 10))),
  ...
  expect_error(
    dispersal_gen$dispersal_friction <- DispersalFriction$new(conductance = raster::stack(replicate(10, raster2))),
...
test_that("calculate dispersals with dispersal friction", {
  ...
  dispersal_gen <- DispersalGenerator$new(
    coordinates = coordinates, distance_classes = seq(100, 400, 20),
...
test_that("error messages for problematic regions", {
  region1 <- Region$new(region_raster = raster::raster(
    vals = 1:16, nrows = 4, ncol = 4,
    xmn = 0, xmx = 400000, ymn = 0, ymx = 400000
  ))
  dispersal_gen <- DispersalGenerator$new(region = region1)
  expect_warning(dispersal_gen$calculate_distance_matrix())
  region2 <- Region$new()
  expect_warning(dispersal_gen <- DispersalGenerator$new(region = region2))
  expect_error(dispersal_gen$calculate_distance_matrix())
})

test_that("default values are correct", {
  coordinates <- data.frame(x = rep(1:4, 4), y = rep(1:4, each = 4))
  dispersal_gen <- DispersalGenerator$new(region = Region$new(coordinates = coordinates, use_raster = FALSE))
  expect_false(dispersal_gen$uses_correlations)
  expect_equal(dispersal_gen$temporal_correlation, 1)
  expect_equal(dispersal_gen$time_steps, 1)
})
