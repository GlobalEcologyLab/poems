...
test_that("setup default function", {
  ...
  expect_equal(environment(dispersal_function)[["dispersal_data_changes"]],
  ...
  for (tm in 1:4) {
    ...
    for (i in 1:2) {
      ...
      for (xi in which(colSums(dispersers) > stage_abundance[i, ])) {
        ...
        for (r in sample(rep(excessive_rows, times = excessive_dispersers),
                         disperser_reduction)) {
