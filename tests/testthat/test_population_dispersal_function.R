...
test_that("user-defined dispersal calculations", {
  simulator <- SimulatorReference$new()
  test_function <- function(params) { # mock dispersal
  ...
  expect_equal(dispersal_function(r = 2, tm = 6, carrying_capacity, stage_abundance, occupied_indices),
  ...
  dispersal_function <- population_dispersal(replicates = 4, time_steps = 10, years_per_step = 1,
                                             ...
                                             dispersal_target_n = list(threshold = 10, cutoff = 15),
                                             simulator)
  expect_error(dispersal_function(r = 2, tm = 6, carrying_capacity, stage_abundance, occupied_indices),
  ...
  expect_warning(dispersal_function(r = 2, tm = 6, carrying_capacity, stage_abundance, occupied_indices),
...
test_that("setup default function", {
  ...
  expected_target_pop_map[as.matrix(dispersal_data[, c("emigrant_row", "source_pop")])] <- dispersal_data$target_pop
  ...
  expect_equal(environment(dispersal_function)[["dispersal_data_changes"]], expected_dispersal_data_changes)
...
test_that("default dispersal calculations", {
  ...
  dispersal_matrix_tm[[4]] <- dispersal_matrix_tm[[4]]*matrix(1/colSums(dispersal_matrix_tm[[4]]), # high dispersal
                                                              nrow = 7, ncol = 7, byrow = TRUE)
  dispersal_gen$dispersal_data[[4]] <- dispersal_gen$dispersal_data[[1]]
  dispersal_gen$dispersal_data[[4]]$dispersal_rate <-
    dispersal_matrix_tm[[4]][as.matrix(dispersal_gen$dispersal_data[[4]][c("target_pop", "source_pop")])]
  dispersal_function <- population_dispersal(replicates = 4, time_steps = 10, years_per_step = 1,
                                             populations = 7, demographic_stochasticity = TRUE,
                                             density_stages = c(0, 1, 1),
                                             dispersal = dispersal_gen$dispersal_data,
  ...
  for (tm in 1:4) {
    ...
    for (i in 1:2) {
      ...
      dispersers[dispersal_indices] <- stats::rbinom(length(dispersal_indices),
                                                     ...
                                                     dispersal_matrix_tm[[tm]][dispersal_indices]*c(0.5, 1)[i])
      ...
      for (xi in which(colSums(dispersers) > stage_abundance[i,])) {
        ...
        for (r in sample(rep(excessive_rows, times = excessive_dispersers), disperser_reduction)) {
  ...
  set.seed(123)
  expect_equal(dispersal_function(r = 2, tm = 1, carrying_capacity_tm_1, stage_abundance, occupied_indices),
               expected_stage_abundance_tm[[1]])
  set.seed(123) # barrier/friction changes
  expect_equal(dispersal_function(r = 2, tm = 2, carrying_capacity_tm_2, stage_abundance, occupied_indices),
               expected_stage_abundance_tm[[2]])
  set.seed(123) # barrier/friction changes reversed
  expect_equal(dispersal_function(r = 2, tm = 3, carrying_capacity_tm_3, stage_abundance, occupied_indices),
               expected_stage_abundance_tm[[3]])
  set.seed(123) # high dispersal => handle excessive dispersers
  expect_equal(dispersal_function(r = 2, tm = 4, carrying_capacity_tm_4, stage_abundance, occupied_indices),
...
test_that("density dependent dispersal", {
  simulator <- SimulatorReference$new()
  region <- Region$new(coordinates = array(c(1:4, 4:1), c(7, 2)))
  dispersal_gen <- DispersalGenerator$new(region = region, dispersal_proportion = 0.6, dispersal_breadth = 110,
                                          dispersal_max_distance = 300, distance_scale = 1000,
                                          distance_classes = seq(100, 400, 20))
  dispersal_gen$calculate_distance_data()
  dispersal_gen$calculate_dispersals(type = "matrix")
  ...
  for (excessive_index in excessive_indices[sample(length(excessive_indices))]) {
    target_indices <- which(dispersal_matrix[, excessive_index] > 0 & density_abundance < 15)
