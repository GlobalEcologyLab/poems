...
test_that("initialization and parameter setting", {
  expect_warning(
    spatial_correlation <- SpatialCorrelation$new(region = Region$new()),
  ...
  expect_error(
    spatial_correlation$correlation_amplitude <- -1,
    "Correlation function parameter amplitude must be between 0 and 1 inclusively"
  )
  expect_error(
    spatial_correlation$correlation_amplitude <- 2,
    "Correlation function parameter amplitude must be between 0 and 1 inclusively"
  )
  expect_error(
    spatial_correlation$correlation_amplitude <- "0",
    "Correlation function parameter amplitude must be numeric"
  )
  expect_error(
    spatial_correlation$correlation_breadth <- -1,
    "Correlation function parameter breadth must be positive/non-zero"
  )
  expect_error(
    spatial_correlation$correlation_breadth <- "0",
...
test_that("calculate distance matrix", {
  ...
  expect_error(
    spatial_correlation$calculate_correlations(),
    "Correlation calculations require region and function parameter settings amplitude and breadth"
  )
  expect_warning(
    spatial_correlation$region <- Region$new(use_raster = FALSE),
    "Spatial region has not been defined within the region object"
  )
  expect_error(
    spatial_correlation$calculate_distance_matrix(),
  ...
  raster2 <- raster::raster(
    vals = spatial_correlation$region$region_raster[], nrows = 4, ncol = 4,
  ...
  expect_warning(
    spatial_correlation$region <- Region$new(region_raster = raster2, use_raster = FALSE),
    "Spatial region has not been defined within the region object"
  )
  spatial_correlation$region$use_raster <- TRUE
...
test_that("calculate correlations", {
  ...
  expect_error(
    spatial_correlation$calculate_correlations(distance_matrix = distance_matrix[1:6, 1:6]),
...
test_that("calculate Cholesky decomposition", {
  ...
  diag(correlation_matrix) <- 1
  spatial_correlation <- SpatialCorrelation$new(
    region = Region$new(coordinates = coordinates), compact_only = FALSE,
  ...
  expect_error(
    spatial_correlation$calculate_cholesky_decomposition(),
...
test_that("calculate compact decomposition", {
  ...
  expect_equal(
    spatial_correlation$t_decomposition_compact_matrix[compact_indices],
...
test_that("generate correlated normal deviates", {
  ...
  expect_equal(
    spatial_correlation$generate_correlated_normal_deviates(random_seed = 1234),
  ...
  expect_equal(
    spatial_correlation$generate_correlated_normal_deviates(random_seed = 1234, time_steps = 5),
    array(correlated_normal_deviates, c(7, 5))
  ) # default temporal_correlation = 1
