...
test_that("raster via coordinates", {
  ...
  raster3 <- raster::raster(
    vals = c(NA, NA, 4, 6, NA, 2, NA, 7, NA, NA, 5, NA, 1, 3, NA, NA),
...
test_that("via template raster", {
  raster2 <- raster::raster(
    vals = c(NA, NA, 1, 1, NA, 1, NA, 1, NA, NA, 1, NA, 1, 1, NA, NA),
    nrows = 4, ncol = 4, xmn = 0, xmx = 4000, ymn = 0, ymx = 4000,
    crs = "+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
  )
  ...
  expect_error(
    region <- Region$new(coordinates = array(c(1:4, 4:1), c(7, 2)), template_raster = raster2),
...
test_that("via region raster", {
  ...
  expect_error(
    region <- Region$new(coordinates = array(c(1:4, 4:1), c(7, 2)), region_raster = raster2),
    "Region should be specified with a set of coordinates or a raster, not both"
  )
