test_that("setup function", {
  ...
  expect_equal(
    environment(density_function)[["growth_rate_max"]],
  ...
  expect_equal(
    environment(density_function)[["multiplier_lookup"]],
    data.frame(
      growth_rate = apply(
        array(multipliers), 1,
  ...
  # Logistic density dependence (stages == 1)
  density_function <- population_density(
    populations = 7, stage_matrix = matrix(1.2), fecundity_mask = matrix(1),
    fecundity_max = NULL, density_dependence = "logistic",
    growth_rate_max = NULL, density_affects = NULL, density_stages = NULL,
    density_precision = NULL, simulator = simulator
  )
...
test_that("calculate and apply multipliers functions", {
  ...
  expect_error(
    density_function <- population_density(
      populations = 7, stage_matrix = stage_matrix,
  ...
  expect_equal(
    multiplier_lookup$growth_rate[nrow(multiplier_lookup)],
    log(Re((eigen(array(c(0, 0.5, 0, 1.333 * 3, 0, 0.7, 4, 0, 0.8), c(3, 3)), only.values = TRUE)$values)[1]))
  )
  expect_equal(
    environment(density_function)[["calculate_multipliers"]](growth_rates = c(0.4, 0.5, 0.6)),
    array(multiplier_lookup$multiplier[apply(
      array(c(0.4, 0.5, 0.6)), 1,
  ...
  expect_equal(
    environment(density_function)[["apply_multipliers"]](transition_array = stage_array,
...
test_that("logistic density dependence", {
  ...
  expect_equal(
    density_function(transition_array = stage_array, carrying_capacity, stage_abundance, occupied_indices),
    expected_stage_array
  )
  # Logistic density dependence (stages == 1)
...
test_that("user-defined density dependence", {
  ...
  logistic_function <- population_density(
    populations = 7, stage_matrix = stage_matrix, fecundity_mask = fecundity_mask,
    fecundity_max = 4, density_dependence = "logistic",
    growth_rate_max = 0.5, density_affects = (1 - fecundity_mask),
    density_stages = c(0, 1, 1), density_precision = NULL, simulator = simulator
  ...
  occupied_indices <- (1:7)[-5]
  expect_equal(
    logistic_function(transition_array = stage_array, carrying_capacity, stage_abundance, occupied_indices)[, , occupied_indices],
    density_function(transition_array = stage_array, carrying_capacity, stage_abundance, occupied_indices)[, , occupied_indices]
  )
  expect_named(simulator$attached$params, c(
    "simulator", "a", "b", "transition_array", "fecundity_mask", "fecundity_max",
  ...
  expect_equal(
    simulator$attached$params[c(
      "a", "b", "transition_array", "carrying_capacity", "stage_abundance",
