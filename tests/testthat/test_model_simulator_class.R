...
test_that("initialization and parameter setting", {
  ...
  expect_error(model_simulator$simulation_function <- file.path(TEST_DIRECTORY, "test_not_a_function.R"),
               paste("Could not assign function", file.path(test_path("test_inputs"), "test_not_a_function.R")))
  expect_error(model_simulator$simulation_function <- file.path(TEST_DIRECTORY, "test_invalid_code.R"),
               "Error loading function from file")
  expect_error(model_simulator$simulation_function <- file.path(TEST_DIRECTORY, "no_such_file.R"),
  ...
  expect_error(model_simulator$simulation_model <- "dummy",
...
test_that("run the simulator", {
  ...
  # Incomplete model
  expect_equal(model_simulator$run(),
  ...
  # Error in simulation
  expect_equal(model_simulator$run()[c("successful", "message")],
               list(successful = FALSE,
  ...
  model_simulator$simulation_function <- function(sim_model) { warning("test"); sim_model$time_steps }
  expect_equal(model_simulator$run()[c("successful", "message")],
  ...
  # Ok simulation
  model_simulator$simulation_function <- function(sim_model) { sim_model$time_steps*5 }
  expect_equal(model_simulator$run(),
               list(successful = TRUE,
