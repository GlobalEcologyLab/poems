...
test_that("initialization and parameter setting", {
  ...
  expect_error(
    generator$occupancy_mask <- array(c(1, 1, 0, 0, 1, 1, 1)),
  ...
  expect_equal(
    generator$get_attributes(c("description", "coordinates", "decimals", "occupancy_mask")),
    list(
      description = "Test generator", coordinates = coordinates, decimals = 4,
...
test_that("file template reading data frame", {
  ...
  expect_warning(
    generator$add_generative_requirements(attr3 = "file", dummy3 = "unknown"),
  ...
  generator$add_file_template("attr3",
    path_template = file.path(TEST_DIRECTORY, "Test_%s_%s_no_such_file.csv"),
  ...
  generator$add_file_template("attr3",
    path_template = file.path(TEST_DIRECTORY, "Test_%s_%s.csv"),
    path_params = c("attr1", "attr2"), file_type = "CSV"
  )
...
test_that("file template reading raster", {
  ...
  # Via GRD (default)
  generator$add_file_template("attr3",
    path_template = file.path(TEST_DIRECTORY, "Test_%s_%s.grd"),
  ...
  # Via RDS
  generator$add_file_template("attr3",
    path_template = file.path(TEST_DIRECTORY, "Test_%s_%s.RData"),
...
test_that("function execution template", {
  ...
  expect_error(
    generator$add_function_template("attr4",
      function_def = file.path(TEST_DIRECTORY, "no_test_function.R"),
  ...
  expect_error(
    generator$add_function_template("attr4",
      function_def = file.path(TEST_DIRECTORY, "test_invalid_code.R"),
      call_params = c("attr1", "attr2")
    ),
  ...
  expect_error(
    generator$add_function_template("attr4",
      function_def = file.path(TEST_DIRECTORY, "test_not_a_function.R"),
      call_params = c("attr1", "attr2")
    ),
    "Could not assign function"
  )
  expect_error(
    generator$add_function_template("attr4",
      function_def = file.path(TEST_DIRECTORY, "test_function_0.R"),
      call_params = c("attr1", "attr2")
    ),
  ...
  expect_error(
    generator$add_function_template("attr4",
      function_def = file.path(TEST_DIRECTORY, "test_function_2.R"),
      call_params = c("attr1", "attr2")
    ),
    "The function definition should have one argument, e.g. function(params)",
    fixed = TRUE
  ...
  generator$add_function_template("attr4",
    function_def = file.path(TEST_DIRECTORY, "test_function_1.R"),
    call_params = c("attr1", "attr2")
  )
...
test_that("sample distribution template (uniform)", {
  ...
  expect_error(
    generator$add_distribution_template("attr5",
      distr_type = "uniform",
  ...
  expect_equal(generator$sample_distribution("attr5"), {
    set.seed(123)
    stats::runif(1, min = 0.2, max = 0.4) * 10
  })
  generator$attached$attr5 <- NULL # clear
  generator$distribution_templates$attr5 <- NULL
  generator$add_distribution_template("attr5",
  ...
  expect_equal(generator$sample_distribution("attr5"), {
    set.seed(123)
    stats::runif(1, min = 0.2, max = 0.6) * 10
  })
  generator$attached$attr5 <- NULL # clear
  ...
  generator$attached$attr5 <- NULL # clear
  expect_equal(generator$get_attribute("attr5"), {
    set.seed(123)
    round(stats::runif(1) * 10, 4)
  })
  generator$attached$attr5 <- NULL # clear
  # Sample as a window
  generator$distribution_templates$attr5 <- NULL
  ...
  expect_equal(generator$sample_distribution("attr5"), {
    set.seed(123)
    stats::runif(1, min = 0.4, max = 0.6) * 10
  })
  generator$attached$attr5 <- NULL # clear
  generator$distribution_templates$attr5 <- NULL
  generator$add_distribution_template("attr5",
  ...
  expect_equal(generator$sample_distribution("attr5"), {
    set.seed(123)
    stats::runif(1, min = 0.1, max = 1) * 10
  })
  generator$attached$attr5 <- NULL # clear
...
test_that("sample distribution template with rasters (uniform)", {
  ...
  expect_equal(generator$attached$attr5[region$region_indices], {
    set.seed(123)
    stats::runif(7) * 10
  })
  ...
  generator$distribution_templates$attr5$distr_params <- list(
    lower = region$region_raster * 0,
    upper = "attr3"
  )
  ...
  expect_equal(unname(generator$sample_distribution("attr5")[region$region_indices]), {
    set.seed(123)
    stats::qunif(stats::runif(7), min = 0, max = attr3_matrix)
  })
  ...
  generator$attached$attr5 <- NULL # clear
  expect_equal(
    unname(generator$get_attribute("attr5")[region$region_indices]),
...
test_that("sample distribution template (normal)", {
  ...
  generator$add_distribution_template("attr5",
    distr_type = "normal", random_seed = 123,
  ...
  expect_equal(unname(generator$sample_distribution("attr5")[region$region_indices]), {
    set.seed(123)
    stats::qnorm(stats::runif(7), mean = attr3_matrix, sd = 0.2)
  })
})
...
test_that("sample distribution template (lognormal)", {
  ...
  generator$add_distribution_template("attr5",
    distr_type = "lognormal", random_seed = 123,
  ...
  expect_equal(unname(generator$sample_distribution("attr5")[region$region_indices]), {
    set.seed(123)
    stats::qlnorm(stats::runif(7), meanlog = attr3_matrix, sdlog = 0.2)
  })
...
test_that("sample distribution template (beta)", {
  ...
  generator$add_distribution_template("attr5",
    distr_type = "beta", random_seed = 123,
  ...
  expect_equal(unname(generator$sample_distribution("attr5")[region$region_indices]), {
    set.seed(123)
    stats::qbeta(stats::runif(7), shape1 = attr3_matrix, shape2 = 0.2)
  })
  expect_equal(unname(generator$attached$attr5[region$region_indices]), {
    set.seed(123)
    stats::qbeta(stats::runif(7), shape1 = attr3_matrix, shape2 = 0.2)
  ...
  limited_sd[available_indices] <- pmin(
    limited_sd[available_indices],
...
test_that("sample distribution template (triangular)", {
  ...
  generator$add_distribution_template("attr5",
    distr_type = "triangular", random_seed = 123,
...
test_that("sample distribution template with correlation (uniform)", {
  ...
  generator$add_generative_requirements(attr5 = "distribution")
  generator$add_distribution_template("attr5",
    distr_type = "uniform", random_seed = 123,
  ...
  expect_equal(
    unname(generator$sample_distribution("attr5")[region$region_indices]),
    stats::qunif(correlated_samples, min = 0, max = attr3_matrix)
  )
  expect_equal(
    unname(generator$attached$attr5[region$region_indices]),
    stats::qunif(correlated_samples, min = 0, max = attr3_matrix)
  )
  generator$attached$attr5 <- NULL # clear
  generator$temporal_correlation <- 0.99
  generator$time_steps <- 10
  correlated_samples <- pnorm(spatial_correlation$generate_correlated_normal_deviates(
    random_seed = 123, time_steps = 10,
...
test_that("sample distribution template with nested aliases", {
  ...
  generator <- Generator$new(
    description = "Test generator", a1 = 1, a2 = 2, region = region,
  ...
  expect_equal(
    unname(generator$get_attribute("a5")[region$region_indices]),
...
test_that("cloning and generation", {
  ...
  region <- Region$new(coordinates = coordinates, use_raster = TRUE)
  generator <- Generator$new(
    description = "Test generator", region = region, decimals = 4,
  ...
  expect_equal(
    generator$generate(input_values = list(a1 = 5, a2 = 7)),
    list(a4 = 12, error_messages = c(
      paste("Error reading file", file.path(test_path("test_inputs"), "Test_5_7.RData")),
  ...
  expect_equal(
    unname(generated_outputs$a5[region$region_indices]),
...
test_that("inheritance", {
  TEST_DIRECTORY <- test_path("test_inputs")
  TestGenerator <- R6::R6Class("TestGenerator",
    inherit = Generator,
    public = list(),
    private = list(
      .model_attributes = c("attr1", "attr2", "attr3"),
      .active_attributes = c(
        "region", "description", "inputs", "outputs", "file_templates", "function_templates",
  ...
  expect_equal(
    generator$generate(input_values = list(a1 = 2, a2 = 1)),
  ...
  expect_equal(
    generator$generate(input_values = list(a1 = 1, a2 = 2)),
