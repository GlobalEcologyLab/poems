...
test_that("setup function", {
  ...
  expect_named(
    environment(transformation_function)[["params"]],
    c(
      "replicates", "time_steps", "years_per_step", "populations", "stages", "demographic_stochasticity",
      "density_stages", "simulator"
    )
  )
  expect_equal(
    environment(transformation_function)[["params"]][c(
      "replicates", "time_steps", "years_per_step",
      "populations", "stages", "demographic_stochasticity",
      "density_stages"
    )],
    list(
      replicates = 4, time_steps = 10, years_per_step = 1, populations = 7, stages = 3,
...
test_that("user-defined transformation", {
  ...
  expect_equal(
    transformation_function(r = 2, tm = 6, carrying_capacity, stage_abundance, occupied_indices),
    list(stage_abundance = expected_stage_abundance)
  )
  expect_named(simulator$attached$params, c(
    "replicates", "time_steps", "years_per_step", "populations", "stages",
    "demographic_stochasticity", "density_stages", "simulator", "a", "b", "r",
    "tm", "carrying_capacity", "stage_abundance", "occupied_indices"
  ))
  expect_equal(
    simulator$attached$params[c("a", "b", "r", "tm", "carrying_capacity", "stage_abundance", "occupied_indices")],
    list(
      a = 1, b = 2, r = 2, tm = 6, carrying_capacity = carrying_capacity,
