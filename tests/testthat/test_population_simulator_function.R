...
test_that("minimal inputs", {
  ...
  for (i in 2:10)
    expected_abundance[i] <- pmin(round(expected_abundance[i - 1] * 1.2), 40)
  expect_equal(population_simulator(inputs),
               list(
                 all = list(abundance = expected_abundance),
...
test_that("results collection", {
  ...
  inputs <- list(
      ...
      harvest = function(params)
        params$stage_abundance - params$r,
  ...
  for (i in 1:3) {
    for (j in 2:10)
      expected_abundance[i, j] <- pmin(round(expected_abundance[i, j - 1] * 1.4), 40) - i
  }
  expect_equal(
    population_simulator(inputs),
    list(
      all = list(
        abundance = array(expected_abundance[1, ]),
  ...
  expect_equal(population_simulator(inputs),
               list(
                 ...
                 abundance = list(
                   mean = array(colMeans(expected_abundance), c(1, 10)),
                 ...
                 harvested = list(
                   mean = array(expected_harvested[2, ], c(1, 10)),
  ...
  expect_equal(
    ...
    list(
      all = list(
        abundance = t(expected_abundance),
...
test_that("module parameter passing and order", {
  ...
  for (i in 2:10)
    expected_abundance[i] <- round(expected_abundance[i - 1] * 1.2)
  expected_density_params <- c(
      "simulator",
      "transition_array",
      "fecundity_mask",
      "carrying_capacity",
  ...
  expect_equal(
    ...
    list(
      all = list(abundance = expected_abundance),
      abundance = array(expected_abundance, c(1, 10)),
      extra_params = expected_transform_params,
      order = inputs$simulation_order[-1],
      dispersal_params = expected_dispersal_params,
      mortality_params = expected_transform_params,
      harvest_params = expected_transform_params,
      transl_params = expected_transform_params,
