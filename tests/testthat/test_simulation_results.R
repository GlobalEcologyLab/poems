...
test_that("initialization and parameter setting", {
  ...
  expect_error(sim_results <- SimulationResults$new(results = file.path(TEST_DIRECTORY, "no_such_file.RData")),
...
test_that("apply occupancy mask and burn-in", {
  ...
  expect_error(sim_results <- SimulationResults$new(time_steps = 12, occupancy_mask = occupancy_mask),
               "The number of occupancy mask layers/columns must be one or match the number of time steps")
  sim_results <- SimulationResults$new(time_steps = 10, occupancy_mask = occupancy_mask,
                                    results = list(abundance = abundance))
  expect_equal(sim_results$get_attributes(), list(time_steps = 10, occupancy_mask = occupancy_mask,
                                                    abundance = abundance*occupancy_mask))
  expect_equal(sim_results$all$get_attributes(), list(time_steps = 10,
  ...
  expect_equal(sim_results$get_attributes()$error_messages,
  ...
  # Checks burn-in
  expect_error(sim_results <- SimulationResults$new(time_steps = 10, burn_in_steps = 10),
               "Burn-in must be less than the number of simulation time steps")
  sim_results <- SimulationResults$new(time_steps = 10, occupancy_mask = occupancy_mask, burn_in_steps = 2,
                                    results = list(abundance = abundance))
  expect_equal(sim_results$get_attributes(), list(time_steps = 10, burn_in_steps = 2, occupancy_mask = occupancy_mask,
                                                    abundance = (abundance*occupancy_mask)[, 3:10]))
  expect_equal(sim_results$all$get_attributes(), list(time_steps = 10, burn_in_steps = 2,
...
test_that("set (region) raster results", {
  ...
  sim_results <- SimulationResults$new(region = region, time_steps = 10,
  ...
  raster::crs(abundance_raster) <- "+proj=utm +ellps=GRS80"
  ...
  expect_equal(sim_results$error_messages[3],
...
test_that("set (region) raster occupancy mask", {
  ...
  occupancy_mask <- array(1, c(5, 10))
  occupancy_mask[2:3, 1:5] <- 0
  expect_error(sim_results <- SimulationResults$new(region = region, time_steps = 10, occupancy_mask = occupancy_mask),
  ...
  expect_error(sim_results <- SimulationResults$new(region = region, time_steps = 10, occupancy_mask = occupancy_mask[1:4,]),
               "The number of occupancy mask rows must be consistent with the region (finite) cells", fixed = TRUE)
...
test_that("apply occupancy mask to raster results", {
  ...
  expect_equal(sim_results$error_messages[1],
...
test_that("apply burn-in to raster results", {
  ...
  coordinates <- data.frame(x = c(1, 3, 2, 2, 3), y = c(3, 3, 2, 1, 1))
  region <- Region$new(coordinates = coordinates)
  sim_results <- SimulationResults$new(region = region, time_steps = 10, burn_in_steps = 2,
                                    results = file.path(TEST_DIRECTORY, "sample_1_raster_results.RData"))
  abundance_raster <- sim_results$attached$abundance
  ...
  expect_equal(sim_results$all$get_attributes(c("abundance", "extra1")),
               list(abundance = colSums(abundance)[3:10], extra1 = extra1)) # burn-in: Yes, No
  expect_null(sim_results$error_messages)
  sim_results$time_steps <- NULL # no steps info
  expect_equal(sim_results$get_attribute("abundance")[], abundance_raster[][, 3:10])
  expect_equal(sim_results$all$get_attributes(c("abundance", "extra1")),
               list(abundance = colSums(abundance)[3:10], extra1 = extra1[3:4])) # burn-in: Yes, Yes
  expect_null(sim_results$error_messages)
  sim_results$time_steps <- 12 # inconsistent steps (would error when abundance set)
  expect_equal(sim_results$get_attribute("abundance")[], abundance_raster[][, 3:10])
  expect_equal(sim_results$all$get_attributes(c("abundance", "extra1")),
               list(abundance = colSums(abundance), extra1 = extra1)) # burn-in: No, No
  expect_null(sim_results$error_messages)
  ...
  expect_equal(sim_results$all$get_attributes(c("abundance", "extra1")),
               list(abundance = colSums(abundance*occupancy_mask[, 1])[3:10], extra1 = extra1[3:4])) # burn-in: Yes, Yes
  expect_null(sim_results$error_messages)
  ...
  expect_equal(sim_results$all$get_attributes(c("abundance", "extra1")),
               list(abundance = colSums(abundance*occupancy_mask)[3:10], extra1 = extra1)) # burn-in: Yes, No
  expect_null(sim_results$error_messages)
})
...
test_that("new cloning", {
  ...
  expect_equal(clone_attributes[c("time_steps", "burn_in_steps", "occupancy_mask")],
  ...
  expect_equal(clone_all_attributes[c("time_steps", "burn_in_steps")],
